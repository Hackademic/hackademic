...
<div class="main_content">
   ...
   <form id="form" name="form" method="post">
      <table class="manager_table">
	    ...
	    <td>
		<select name="modules" class="width_90">
			    <option value="default">Add modules</option>
			    {foreach from=$modules_not_assigned item=module}
	...
	{foreach from=$modules item=module}
	    <tr>
		<td>{$module->name}</td>
		<td><a href="{$site_root_path}?url=admin/showclass&mid={$module->id}&id={$class->id}&action=del">Remove</a></td>
		</tr>
	{/foreach}
    </table>
