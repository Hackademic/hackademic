 ...
 *
 * This class contains the plugin API that 3rd party plugins can use to add actions
 * and filters that the Hackademic core executes.
 *
 * The whole class is based on the Wordpress Plugin API
 * (http://codex.wordpress.org/Plugin_API/) with very few modifications to suit the
...
class Plugin
{
   ...
   *                                  $function_to_add to.
   * @param callback $function_to_add The name of the function to be called when the
   *                                  filter is applied.
   * @param int      $priority        optional. Used to specify the order in which
   *                                  the functions associated with a particular
   *                                  action are executed (default: 10). Lower
   *                                  numbers correspond with earlier execution,
   *                                  and functions with the same priority are
   ...
   * @global array $merged_filters Tracks the tags that need to be merged for later.
   ...
   * @param array  $args The arguments supplied to the functions hooked
   *                     to <tt>$tag</tt>
   *
   ...
   *
   * @param string   $tag                The filter hook to which the function to be
   *                                     removed is hooked.
   * @param callback $function_to_remove The name of the function which should be
   *                                     removed.
   * @param int      $priority           optional. The priority of the function
   *                                     (default: 10).
   *
   ...
   *                                  accept (default 1).
   *
   * @uses add_filter() Adds an action. Parameter list and functionality are the
   *                    same.
   *
   * @return boolean true
   */
   ...
   * hook and possibly replace them with a substitute.
   *
   * @param string   $tag                The action hook to which the function to be
   *                                     removed is hooked.
   * @param callback $function_to_remove The name of the function which should be
   *                                     removed.
   * @param int      $priority           optional The priority of the function
   ...
   * @param Path $file the new path to the file that generates the page view.
   ...
   *
   * @param URL $url the url to delete the page mapping for.
   *
   * @return true if deleted
   */
   ...
   * @param Label   $label  the label for the menu item that is visible to the user
   * @param ID      $parent the parent menu item id if there is one, otherwise 0 if
   *                        root item
   * @param integer $sort   the sort integer for the menu item, sort is made
   *                        ascending  
   *
   * @return true if added
   */
   ...
   *                           If === false and $function is an object reference, we
   *                           return the unique id only if it already has one, false
   *                           otherwise.
   *
