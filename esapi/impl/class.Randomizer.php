 ...
 *
 * The ESAPI is published by OWASP under the BSD license. You should read and accept
 * the LICENSE before you use, modify, and/or redistribute this software.
 *
 * PHP Version 5.
 *
...
class Randomizer_impl implements Randomizer
{
	 ...
	 * cryptographically strong pseudo-random number generator. If SecureRandom is
	 * not used, the pseudo-random number gernerator used should comply with the
	 ...
	 * 4.9.1.
     *
     * @param length       $numChars the length of the string
     * @param characterSet $charset  the set of characters to include in the created
	 *                               random string
	 *
	 ...
	 * gernerator used should comply with the statistical random number generator
	 ...
	 * 4.9.1.
     *
     * @param min $min the minimum integer that will be returned
     * @param max $max the maximum integer that will be returned
     *
	 ...
	 * 4.9.1.
     *
	 * mt_rand() without arguments will return between 0 and mt_getrandmax(). That's
	 * about as good as PHP gets
     *
	 ...
	 * method could call getRandomString(length, charset) from this Class with the
	 * desired length and alphanumerics as the charset then merely append "." +
	 * extension.
     *
     * @param extension $extension extension to add to the random filename
     *
     * @return a random unguessable filename ending with the specified extension
     */
    function getRandomFilename($extension = '')
    {
		// Because PHP runs on case insensitive OS as well as case sensitive OS, only
		// use lowercase
	 ...
	 * 4.9.1.
     *
     * @param min $min the minimum real number that will be returned
     * @param max $max the maximum real number that will be returned
     *
    ...
    function getRandomReal($min, $max)
	{
        ...
        $rf = (float) (mt_rand() / $this->_maxRand);
    ...
    /**
	 * Generates a random GUID.  This method could use a hash of random Strings, the
	 * current time, and any other random data available.  The format is a
    ...
    function getRandomGUID()
    {
		...
		mt_rand(0, 4095),  // 12 bits before the 0100 of
		                   //(version) 4 for "time_hi_and_version"
        					 bindec(substr_replace(sprintf('%016b', mt_rand(0, 65535)), '01', 6, 2)),
							 // 8 bits, the last two of which (positions 6 and 7)
							 // are 01, for "clk_seq_hi_res"
							 // (hence, the 2nd hex digit after the 3rd hyphen can
							 // only be 1, 5, 9 or d)
