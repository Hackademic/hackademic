...
class MenuManagerController extends HackademicBackendController
{
    ...
    private static $_action_type = 'menu_manager';
    ...
    public function go()
    {
        ...
        if ($submit == 'Cancel' || empty($submit)) {
            ...
            // Get the menu that has been shown
            // (can be different from the selected menu)
            ...
            foreach ($_POST as $key => $value) {
                ...
                // When we have a parent we can get the id which is the same for the
                // parent and sort select lists.
                if ($is_parent) {
                    $pieces = explode('-', $key);
                    $id = $pieces[1];
                    $menu_item = $menu->items['items'][$id];

                    // Make sure the selected menu is the same as the menu that was
                    // shown
                    if ($menu_item['mid'] == $_POST['menu']) {
                        ...
                        if (isset($_POST['parent-' . $value]) && $id == $_POST['parent-' . $value]) {
                            $this->addErrorMessage(
                                "A child element cannot be parent
                                 to its own parent."
                            );
                        }

                        // If we have new and valid data, lets store it in the
                        // database
                    ...
                    // The user selected another menu and tried to update item
                    // without first submitting the change of menu
                    else {
                            $this->addErrorMessage(
                                "The selected menu did not correspond
							     to the items you where editing. Did you remember to
            ...
            // We have saved all item changes if any, lets fetch a fresh menu from
            // the database to make sure we get the structure in the way we want it
            // in the template.
            $menu = Menu::getMenu(htmlspecialchars($_POST['menu']));
        }
