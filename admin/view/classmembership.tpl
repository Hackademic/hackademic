...
<div class="main_content">
	...
	<div class="page_title"><h3 class="left">{t}Class Membership - Users {/t}{$user->username}</h3></div>
    </div><br/>
    ...
    <div id="input_form">
       <form method="post">
	    <table class="add_form">
	        <tr>
		    <td><label> {t}Add user to class: {/t}</label></td>
		    <td>
	...
	{foreach from=$class_memberships item=class}
		...
		<td><a href="{$site_root_path}?url=admin/classmemberships&id={$smarty.get.id}&class_id={$class['class_id']}&action=del">{t}Delete{/t}</a></td>
	    </tr>
	{/foreach}
    </table>
