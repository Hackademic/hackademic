...
class Install:
    ...
    def install_docker(self):
        ...
        try:
            ...
            try:
                subprocess.check_call("wget -qO- https://get.docker.com/ | sudo sh", stdout=subprocess.PIPE,
                                      stderr=subprocess.STDOUT, shell=True)

                subprocess.check_call("sudo usermod -aG docker " + self.user, stdout=subprocess.PIPE,
                                      stderr=subprocess.STDOUT, shell=True)

            except (OSError, subprocess.CalledProcessError) as exception:
                self.install.install_log('Installation Error: \n' + str(exception))
                exit("[+] Unknown installation error happened. Check logs for more details...")
    ...
    def docker_image(self):
        ...
        print("[+] Now Pulling Image File . This could take sometime. Please wait...")

        try:
            subprocess.check_call("docker pull phusion/baseimage:latest", stdout=subprocess.PIPE,
                                  stderr=subprocess.STDOUT, shell=True)

        except (OSError, subprocess.CalledProcessError) as exception:
            self.install.install_log('Image File Download Error: \n' + str(exception))
            exit("[+] Image File Download Interrupted. Check logs for more details...")

    def build_docker(self):

        """
        This will build the docker image with the specified Dockerfile which adds all
        the challenges and install necessary applications for running the challenges.

        Once building is over, try the command " docker images " and if you can see an
        image named 'Hackademic', installation is successful.

        """
        print("[+] Building and Configuring Docker")
        subprocess.call("docker rmi -f hackademic", stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
        try:
            subprocess.check_call("docker build -t hackademic .", stdout=subprocess.PIPE,
                                  stderr=subprocess.STDOUT, shell=True)
